...
static void XferToFifoWR16(spi_transfer_16t *xfer, uint32_t *fifowr)
{
    *fifowr |= ((xfer->configFlags & (uint32_t)kSPI_FrameDelay) != 0U) ? (uint32_t)kSPI_FrameDelay : 0U;
    *fifowr |= ((xfer->configFlags & (uint32_t)kSPI_FrameAssert) != 0U) ? (uint32_t)kSPI_FrameAssert : 0U;
}
...
static void PrepareTxLastWord16(spi_transfer_16t *xfer, uint32_t *txLastWord, spi_config_t *config)
{
...
static void PrepareTxLastWord17(spi_transfer_16t *xfer, uint32_t *txLastWord, spi_config_t *config)
{
    *txLastWord = xfer->txData[xfer->dataSize/(sizeof(uint16_t)) - 1U];
...
static void SPI_SetupDummy16(SPI_Type *base, spi_dma_txdummy_t *dummy, spi_transfer_16t *xfer, spi_config_t *spi_config_p)
{
    uint32_t instance  = SPI_GetInstance(base);
    uint32_t dummydata = (uint32_t)s_dummyData[instance];
    dummydata |= (uint32_t)s_dummyData[instance] << 8U;
    ...
    XferToFifoWR16(xfer, &dummy->word);
    XferToFifoWR16(xfer, &dummy->lastWord);
    SpiConfigToFifoWR(spi_config_p, &dummy->word);
    SpiConfigToFifoWR(spi_config_p, &dummy->lastWord);
    /* Clear the end of transfer bit for continue word transfer. */
    dummy->word &= (~(uint32_t)kSPI_FrameAssert);
...
status_t SPI_MasterTransferDMA16MemInc(SPI_Type *base, spi_dma_handle_t *handle, spi_transfer_16t *xfer)
{
 assert(!((NULL == handle) || (NULL == xfer)));
    ...
    else
    {
        ...
        DMA_PrepareTransfer(&xferConfig, (uint32_t *)address, &s_rxDummy,(sizeof(uint16_t)),
        ...
        {
            PrepareTxLastWord17(xfer, &s_txLastWord[instance], spi_config_p);
        }
                ...
                DMA_PrepareTransfer(
                &xferConfig, xfer->txData, (uint32_t *)address, sizeof(uint16_t),
...
status_t SPI_MasterTransferDMA16MemNotInc(SPI_Type *base, spi_dma_handle_t *handle, spi_transfer_16t *xfer)
{
  assert(!((NULL == handle) || (NULL == xfer)));
    ...
    else
    {
        ...
        if ((xfer->configFlags & (uint32_t)kSPI_FrameAssert) != 0U)
        {
            PrepareTxLastWord16(xfer, &s_txLastWord[instance], spi_config_p);
        }
