...
void Servo::write(int value) {
  if (value < MIN_PULSE_WIDTH) { // treat values less than 544 as angles in degrees (valid values in microseconds are handled as microseconds)
    value = LPC5528::util::map(std::clamp(value, 0, 180), 0, 180, SERVO_MIN(), SERVO_MAX());
...
int Servo::read() { return LPC5528::util::map(this->readMicroseconds() + 1, SERVO_MIN(), SERVO_MAX(), 0, 180); }
