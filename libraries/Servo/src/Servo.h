...
typedef struct {
  int16_t nbr;// pin_t nbr;            // a pin number from 0 to 254 (255 signals invalid pin)
...
class Servo {
  public:
    Servo();
    int8_t attach(const int16_t pin);    // attach the given pin to the next free channel, set pinMode, return channel number (-1 on fail)
    int8_t attach(const int16_t pin, const int min, const int max); // as above but also sets min and max values for writes.
    void detach();
